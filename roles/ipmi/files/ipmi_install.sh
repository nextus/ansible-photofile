...
if [[ -z $IPMI_PASS ]]; then
    read -s -p "Enter desired password for IPMI management: " IPMI_PASS
fi
...
initIPMI() {
    ...
    $IPMI user set password 2 $IPMI_PASS
...
# not ready yet, isn't using in program right now
...
SOL_linux() {
    ...
    # 6th centos version has upstart init daemon instead of SysV
    CENTOS=$1
...
SOL_freebsd() {
    ...
    ID=$(vmstat -ai | grep uart | head -1 | cut -d' ' -f2 | cut -c5)
    if [[ -z ${ID} ]]; then
        echo "Couldn't found any of the serial ports"
        exit 1
    fi
...
case $OS in 
    Linux)
        # get centos major and minor versions
        CENTOS_VERSION=$(rpm -qa centos-release)
        if [[ $? -ne 0 ]] || [[ -z ${CENTOS_VERSION} ]]; then
            echo "Couldn't determine specific CentOS version"
            exit 1
        fi
        MAJOR_VERSION=$(echo ${CENTOS_VERSION} | cut -d"-" -f3)
        MINOR_VERSION=$(echo ${CENTOS_VERSION} | cut -d"-" -f4 | cut -d'.' -f1)

        # kernel in 6.4 and greather is comprised ipmi modules statically
        if [[ ! ( ${MAJOR_VERSION} -eq "6" && ${MINOR_VERSION} -ge "4" ) ]]; then
